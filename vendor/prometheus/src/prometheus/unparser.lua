...
function Unparser:unparseExpression(expression, tabbing)
	...
	if(expression.kind == AstKind.NumberExpression) then
		...
		-- LuaLune patch (upstream fix pending): tostring() formats numbers with the
		-- host's "%.14g", so any literal carrying more than 14 significant digits
		-- was silently truncated on the way out (3.3333333333333335 became
		-- 3.3333). Re-emit such values losslessly.
		if(type(expression.value) == "number" and tonumber(str) ~= expression.value) then
			str = string.format("%.17g", expression.value);
		end
