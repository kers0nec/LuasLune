...
-- LuaLune patch (upstream fix pending): every check in this file used to run with
-- the host interpreter's exact 64-bit integers, while the emitted code is executed
-- by Lua 5.1 or Luau, where all numbers are doubles. Expressions whose intermediate
-- values exceed 2^53 therefore verified fine in the host and still evaluated to a
-- different number at runtime, which produced builds that decoded their own strings
-- into garbage. Forcing the operands through a float (+ 0.0) makes the host compute
-- with exactly the arithmetic the generated code will use.
local function toDouble(n)
	return n + 0.0
end
...
function NumbersToExpressions:init(_)
	self.ExpressionGenerators = {
		function(val, depth) -- Addition
			...
			local diff = val - val2
			if toDouble(val2) + toDouble(diff) ~= toDouble(val) then
				return false
			end
		...
		function(val, depth) -- Subtraction
			local val2 = math.random(-2 ^ 20, 2 ^ 20)
			local diff = val + val2
			if toDouble(diff) - toDouble(val2) ~= toDouble(val) then
				return false
			end
		...
		function(val, depth) -- Modulo
			...
			if toDouble(lhs) % toDouble(rhs) ~= toDouble(val) then
				return false
			end
...
function NumbersToExpressions:CreateNumberExpression(val, depth)
	if depth > 0 and math.random() >= self.InternalThreshold or depth > 15 then
		...
		-- LuaLune patch (upstream fix pending): a rewritten representation is only
		-- safe when the target's doubles hold the value exactly.
		if toDouble(val) ~= val then
			return Ast.NumberExpression(val)
		end
		...
		if format == "scientific" then
			...
			-- LuaLune patch (upstream fix pending): "%.15g" cannot round-trip every
			-- IEEE-754 double, so this mutation silently changed the value of a
			-- constant (3.3333333333333335 became 3.33333333333333) and produced
			-- builds that printed garbage. "%.17g" round-trips doubles, and the
			-- comparison below makes sure a wrong literal is never emitted.
			local literal = string.format("%.17ge%d", mantissa, exp)
			if tonumber(literal) ~= val then
				return Ast.NumberExpression(val)
			end
			return Ast.NumberExpression(literal)
		end
