...
function supabaseStore(url, anonKey, serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY) {
  const clientFor = (token) => createClient(url, anonKey, {
    global: { headers: token ? { Authorization: `Bearer ${token}` } : {} },
    auth: { persistSession: false, autoRefreshToken: false },
  });
  const service = serviceKey ? createClient(url, serviceKey, { auth: { persistSession: false, autoRefreshToken: false, detectSessionInUrl: false } }) : clientFor(null);
